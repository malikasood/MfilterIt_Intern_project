...
#to get the bounding box for labels that have been detected in the previous step,but using condition in confidence  
    if results!= []:
      for k in results:
        if k['name'] == 'veg' and k['confidence'] >= 0.55:
            food_label.append(k['name'])
            food_label.append(k['confidence'])
        elif k['name'] == 'non_veg' and k['confidence'] >= 0.65:
            food_label.append(k['name'])
            food_label.append(k['confidence'])
...
#cv2.imwrite()
        ...
        cv2.imwrite("C:/Users/Malika Sood/Desktop/Intern/Mfilterit/Project/TRAINED DATA food_label_detection/images/train/test_food_labels_images/"+ image_name,image)
